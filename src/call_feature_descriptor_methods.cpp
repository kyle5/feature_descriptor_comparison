...
extern "C" void initcall_feature_descriptor_methods(void) {
	Py_InitModule( "call_feature_descriptor_methods", passing_list_methods );
}
...
static vector<vector<DMatch> > match_features_from_images( vector<Mat> all_image_features ) {
	...
	for(int i = 0; i < all_but_one; i++ ) {
		...
		if ( (int) matches.size() == 0 ) throw runtime_error( "No matches! Got to meltdown sooner or later!" );
